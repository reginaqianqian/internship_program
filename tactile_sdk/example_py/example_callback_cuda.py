...
def callback(frame):
    ...
    if frame['content'].get('F6') is not None: f6 = frame['content']['F6'].squeeze().copy()
    if frame['content'].get('CONTACT_POINT') is not None: p = frame['content']['CONTACT_POINT'].squeeze()
    data[ch] = img, f6, deform, p
